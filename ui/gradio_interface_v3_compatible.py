...
def submit_pdf_with_tracking(pdf_file, publication, edition, language, zone, date, dpi, quality, resize_bool, base_url):
    ...
    if pdf_file is None:
        return "❌ No PDF file selected", "No processing active"
    ...
    try:
        ...
        # Open and read the PDF file properly for Gradio 3.x
        with open(pdf_file.name, 'rb') as f:
            files = {"pdf": (os.path.basename(pdf_file.name), f, "application/pdf")}
            data = {
        ...
        if resp.status_code == 200 and "task_id" in response_data:
            ...
            return f"❌ Error {resp.status_code}: {resp.text}", "Error occurred"
            
    except Exception as e:
        processing_state["status"] = "error"
        return f"❌ Exception: {str(e)}", "Error occurred"
...
def get_processing_status():
    ...
    if processing_state["status"] == "idle":
        return "⏸️ Idle - No active processing", create_results_display([])
...
- **Status**: {processing_state['status'].title()}"""
    ...
    return status_text, results_display

def create_results_display(articles: List[Dict]) -> str:
    """Create formatted analysis results display"""
    if not articles:
        return "No articles processed yet"
    
    results_html = f"<div style='max-height: 500px; overflow-y: auto;'><h3>📰 Processed Articles ({len(articles)})</h3>"
...
def build_interface():
    ...
    with gr.Tab("📄 PDF Processing Dashboard"):
        gr.Markdown("## Production OCR Pipeline Dashboard")
        
        with gr.Row():
            # Left column - Input form (compact)
            ...
            with gr.Column(scale=2):
                ...
                current_status = gr.Textbox(label="Current Status", value="Waiting for input", interactive=False)
                
                # Results display
                results_display = gr.HTML(label="Processing Results")
                
                # Manual refresh button (since auto-refresh requires Gradio 4.0+)
        ...
        pdf_btn.click(
            ...
            outputs=[status_display, current_status]
        )
...
def main():
    with gr.Blocks(
        ...
        gr.Markdown(
            """
            # 🏭 Production OCR Pipeline Dashboard
            
            **Streamlined newspaper processing pipeline** with real-time visualization and analytics.
            
            **Pipeline Steps**: PDF Upload → Image Conversion → Article Segmentation → AI Analysis → Results
            """
        )
        ...
        gr.Markdown(
            ...
            - Ensure clear PDF quality for optimal segmentation
