...
- **Status**: {processing_state['status'].title()}"""
    ...
    analytics_display = create_simple_analytics(processing_state.get("articles", []))
    
    return status_text, processing_state["progress"], processing_state["step"], results_display, analytics_display
...
def create_simple_analytics(articles):
    ...
    <div style='background: #f8fafc; padding: 15px; border-radius: 8px;'>
        <h3 style='color: #1f2937; margin-top: 0;'>📊 Analytics Summary</h3>
        <div style='display: grid; grid-template-columns: 1fr 1fr; gap: 15px;'>
            <div>
                <h4 style='color: #374151; margin-bottom: 8px;'>📈 Summary</h4>
                <p><strong>Total Articles:</strong> {len(articles)}</p>
                <p><strong>Unique Ministries:</strong> {len(ministry_counts)}</p>
            </div>
            <div>
                <h4 style='color: #374151; margin-bottom: 8px;'>😊 Sentiment</h4>
                {' '.join([f"<p><strong>{k}:</strong> {v}</p>" for k, v in sentiment_counts.items()])}
            </div>
        </div>
    </div>
    """
...
def main():
    with gr.Blocks(title="OCR Pipeline Dashboard") as demo:
        ...
        with gr.Tab("📄 PDF Processing"):
                ...
                with gr.Column(scale=2):
                    ...
                    with gr.Tabs():
                        ...
                        with gr.Tab("📊 Analytics"):
                            analytics_display = gr.HTML("No analytics yet")
            ...
            pdf_btn.click(
                ...
                outputs=[status_display, progress_bar, step_info, results_display, analytics_display]
            )
